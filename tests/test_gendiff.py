...
@pytest.mark.parametrize("file1,file2,format,expected", testdata_stylish)
def test_generate_diff_stylish(file1, file2, format, expected):
    assert generate_diff(file1, file2, format) == expected


@pytest.mark.parametrize("file1,file2,format,expected", testdata_plain)
